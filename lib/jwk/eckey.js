...
var ecutil = require("../algorithms/ec-util.js"),
    ...
    pluggedData = require('../plugin/customECPlug.js');
...
var PREFERRED_SIG_ALGO = {
  "P-256" : "ES256",
  "P-384" : "ES384",
  "P-512" : "ES512"
}

 pluggedData.PREFERRED_SIG_ALGO.forEach(function(item){
  PREFERRED_SIG_ALGO[item.curve] = item.algo;
})
...
function oidToCurveName(oid) {
  switch (oid) {
    ...
    case "1.3.132.0.35":
      return "P-521";
    default:
      if(pluggedData.oidToCurveName[oid]!=undefined)
        return pluggedData.oidToCurveName[oid];
      return null;
  }
}
function curveNameToOid(crv) {
  switch (crv) {
    ...
    case "P-521":
      return "1.3.132.0.35";
    default:
      if(pluggedData.curveNameToOid[crv]!=undefined)
        return pluggedData.curveNameToOid[crv];
      return null;
  }
}
    ...
    switch (mode) {
      ...
      case "sign":
        if (!keys.private) {
         ...
         return PREFERRED_SIG_ALGO[keys.private.crv];
      case "verify":
        if (!keys.public) {
          return [];
        }
        return PREFERRED_SIG_ALGO[keys.public.crv];
    }
  },
