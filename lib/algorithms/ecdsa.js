...
function ecdsaSignFN(curve,hash) {
  
  if(hash==undefined){
    hash = curve;
    curve = idealCurve(hash);
  }
...
function ecdsaVerifyFN(curve,hash) {
  if(hash==undefined){
    hash = curve;
    curve = idealCurve(hash);
  }
...
pluggedData.ECDSA_ALGO.forEach(function(alg){
  ecdsa[alg.name] = {
    sign:  ecdsaSignFN(alg.curve,alg.hash),
    verify: ecdsaVerifyFN(alg.curve,alg.hash)
  };
})
